...
- **迁移历史已压缩成单文件**：`src/db/migrations/` 下**只有** `1789700000000-InitSchema.ts`，它描述当前完整结构，全新库一次建好；不再有存量库收敛迁移。结构变更请直接改它（本阶段库一律重建，不做存量库收敛）。`up()` 仍会先探测 schema 是否已存在，存在就跳过建表并被记录为已执行（反过来 `down()` 也会判断「之前是否还有已执行的迁移」，避免在不该拆的库上整库 DROP——两条分支由 `src/db/init-schema.migration.spec.ts` 钉住）。
