...
/**
 * 清理被取代的冗余索引——**专为存量库收敛**：初始化迁移在存量库上只做 baseline
 * （`up()` 探测到 schema 已存在就跳过整段 DDL），因此新库里「没有这些索引」这一事实
 * 不会自动落到存量库上。单独一个迁移同时作用于新库（空转）与存量库（真正删索引）。
 *
 * 三条都是纯写放大，删除不影响任何查询（计数/删除/级联都按前缀走那两个复合索引 / 唯一索引）：
 * - `IDX_knowledge_documents_kb`（knowledge_documents.knowledge_base_id）：
 *   已被 `IDX_knowledge_documents_kb_created_id` 的前导列完全覆盖；
 * - `IDX_knowledge_documents_kb_created`（knowledge_base_id, created_at）：
 *   缺 `id` 决胜列，无法覆盖 keyset 的 `ORDER BY created_at DESC, id DESC`，
 *   已由 `IDX_knowledge_documents_kb_created_id` 取代；
...
const REDUNDANT_INDEXES = [
  ...
  {
    name: 'IDX_knowledge_documents_kb_created',
    table: 'knowledge_documents',
