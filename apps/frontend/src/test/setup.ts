...
// 测试期间显式指定 API baseURL 为 '/api/v1/'，与 vite dev 服务器（带 /api proxy +
// 后端 URI 版本前缀 /v1）行为一致。被 client.ts 顶层读取，必须在 client.ts 导入前
// 赋值，否则 baseURL 已被解析。setupFiles 在每个测试文件 import 之前执行，时机合适。
(globalThis as { __lucyApiBaseUrl?: string }).__lucyApiBaseUrl = '/api/v1/';
