...
export interface paths {
    ...
    "/ai/conversations": {
        ...
        /**
         * 会话列表
         * @description 按最近活跃倒序，游标分页。排序键是**可变列** updatedAt：翻页期间被更新的会话会被移到 游标之前，本轮翻页取不到（不会重复，但需刷新或重拉首页才会出现）——客户端在写操作（发消息/改名）后应重置到首页。nextCursor 为 null 即末页（list 为空只能是「游标之后的行被更新/删除」的兜底情形，满页的末页不会给出下一次游标）
         */
